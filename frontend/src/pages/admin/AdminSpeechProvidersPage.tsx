...
export function AdminSpeechProvidersPage() {
  ...
  return <section className="flex flex-1 flex-col text-[#1d1a3d]">
    <div className="border-b border-indigo-950/10 pb-6"><p className="text-xs font-bold uppercase tracking-wider text-teal-700">Admin dashboard</p><h1 className="mt-2 text-3xl font-extrabold text-[#1d1a5e]">{text.title}</h1></div>
    ...
    {isLoading ? <p className="py-12 text-center font-semibold">{text.loading}</p> : draft && dashboard ? <>
      ...
      <div className="mt-6 flex flex-wrap items-end justify-between gap-4 border-b border-indigo-950/10">
        <div className="flex gap-1" role="tablist" aria-label="Speech provider views">
          {(["routing", "activity"] as const).map((view) => <button key={view} type="button" role="tab" aria-selected={activeView === view} onClick={() => setActiveView(view)} className={`border-b-2 px-4 py-3 text-sm font-bold capitalize ${activeView === view ? "border-[#2a2586] text-[#2a2586]" : "border-transparent text-slate-400 hover:text-[#2a2586]"}`}>{view}</button>)}
        </div>
...
function ProviderOverview({ dashboard, text, labels, language }: { dashboard: GlobalSpeechDashboard; text: ReturnType<typeof getText>; labels: Record<string, string>; language: LanguageCode }) {
  ...
  const cards = [
    { label: "Active STT provider", value: providerName(dashboard.active_stt_provider), detail: `${labels.priority} ${activeStt?.priority ?? "-"} · ${activeStt?.health_status ?? "-"}` },
    { label: "Active TTS provider", value: providerName(dashboard.active_tts_provider), detail: `${labels.priority} ${activeTts?.priority ?? "-"} · ${activeTts?.health_status ?? "-"}` },
    { label: sttUsageLabel(language), value: `${sttMinutes.toLocaleString(text.locale, { maximumFractionDigits: 2 })} ${fullMinuteLabel(language)}`, detail: acrossProvidersLabel(language) },
    { label: "TTS characters (30d)", value: ttsCharacters.toLocaleString(text.locale), detail: "Across all providers" },
  ];
...
function CompactProviderSection({ service, draft, labels, text, language, updateCapability, move }: ProviderSectionProps) {
  ...
  return <section className="mt-5">
    <div className="rounded-xl border border-indigo-950/10 bg-white px-6 py-5">
      <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Failover order · {service}</p>
      <div className="mt-4 flex flex-wrap items-center gap-3">{items.map((item, index) => <div key={item.provider_key} className="contents"><span className={`rounded-lg px-4 py-3 text-sm font-bold ${item.provider_key === active ? "border border-cyan-300 bg-cyan-50 text-[#2a2586]" : "bg-[#f3f3fb] text-slate-600"}`}><i className={`mr-2 inline-block h-2 w-2 rounded-full ${item.provider_key === active ? "bg-cyan-400" : "bg-slate-400"}`} />{providerName(item.provider_key)}{item.quota_type === "unlimited" ? ` (${labels.unlimited})` : ""}</span>{index < items.length - 1 && <span className="text-slate-400">→</span>}</div>)}</div>
    </div>
    <div className="mt-4 overflow-hidden rounded-xl border border-indigo-950/10 bg-white">{items.map((item, index) => <article key={item.provider_key} className={`border-b border-indigo-950/10 last:border-b-0 ${providerRowStyle(item.provider_key, item.provider_key === active)}`}>
      <div className="grid items-center gap-4 px-5 py-5 md:grid-cols-[44px_minmax(190px,1.5fr)_minmax(150px,1fr)_110px_auto]">
        ...
        <div><Metric label={text.used} value={item.quota_type === "unlimited" ? `${item.used} ${text.requests}` : `${formatCapabilityAmount(item.used, service, text.locale)} / ${formatCapabilityAmount(item.quota_limit ?? 0, service, text.locale)} ${service === "stt" ? minuteUnit : item.usage_unit} · ${(item.usage_percent ?? 0).toFixed(1)}%`} /><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/80 ring-1 ring-indigo-950/5"><div className={`h-full rounded-full ${providerBarStyle(item.provider_key)}`} style={{ width: `${item.quota_type === "unlimited" ? 0 : Math.max(1, Math.min(100, item.usage_percent ?? 0))}%` }} /></div></div>
        <div><p className="text-xs font-bold uppercase text-slate-500">{text.status}</p><span className={`mt-2 inline-flex rounded-full px-3 py-1 text-xs font-bold ${providerStatusStyle(item.quota_status, item.quota_type)}`}>{item.quota_type === "unlimited" ? labels.unlimited : item.quota_status}</span></div>
      ...
      <details className="border-t border-indigo-950/5 bg-white/70">
        <summary className="cursor-pointer px-5 py-3 text-sm font-bold text-[#3730a3] hover:bg-cyan-50/60">Configure provider ›</summary>
...
function CompactHistory({ dashboard, title, eventTitle, locale }: { dashboard: GlobalSpeechDashboard; title: string; eventTitle: string; locale: string }) {
  ...
  return <div className="mt-6">
    <UsageComparison dashboard={dashboard} locale={locale} />
    <div className="mt-5 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
    <ActivityList title={title} page={usagePage} total={dashboard.usage_history.length} pageSize={pageSize} onPage={setUsagePage}>{usageItems.map((item) => <div key={`${item.billing_period}-${item.provider}-${item.service_type}`} className="flex items-center justify-between border-b border-slate-100 py-3 text-sm last:border-0"><div><strong>{item.provider.toUpperCase()}</strong><span className="ml-2 text-xs font-bold text-slate-500">{item.service_type.toUpperCase()}</span><p className="text-xs text-slate-500">{new Date(`${item.billing_period}T00:00:00`).toLocaleDateString(locale)}</p></div><strong>{item.service_type === "tts" ? item.characters_used.toLocaleString(locale) : `${formatMinutes(item.audio_seconds_used, locale)} ${fullMinuteLabelFromLocale(locale)}`}</strong></div>)}</ActivityList>
    <ActivityList title={eventTitle} page={eventPage} total={dashboard.events.length} pageSize={pageSize} onPage={setEventPage}>{eventItems.map((item) => <div key={item.id} className="border-b border-slate-100 py-3 pl-4 text-sm last:border-0" style={{ borderLeft: `3px solid ${item.service_type === "tts" ? "#2dd8d8" : "#3730a3"}` }}><div className="flex items-start justify-between gap-3"><div><strong className="capitalize">{item.event_type.replace(/_/g, " ")}</strong><p className="mt-0.5 text-xs text-slate-500">{formatDate(item.created_at, locale)}{item.administrator_name ? ` · ${item.administrator_name}` : ""}</p></div><span className="rounded-md bg-[#f3f3fb] px-2 py-1 text-xs font-bold text-[#2a2586]">{item.service_type.toUpperCase()}</span></div>{(item.previous_provider || item.new_provider) && <p className="mt-2 text-xs font-bold text-teal-700">{providerName((item.previous_provider ?? item.new_provider) as GlobalSpeechProvider)} → {providerName((item.new_provider ?? item.previous_provider) as GlobalSpeechProvider)}</p>}<p className="mt-2 text-sm leading-5 text-slate-700">{item.reason}</p></div>)}</ActivityList>
    </div>
  </div>;
}

function UsageComparison({ dashboard, locale }: { dashboard: GlobalSpeechDashboard; locale: string }) {
  ...
  return <section className="grid gap-8 rounded-xl border border-indigo-950/10 bg-white p-7 md:grid-cols-2">
    <UsageBars title={`STT usage · ${fullMinuteLabelFromLocale(locale)} this period`} values={providers.map((item) => ({ name: providerName(item.provider), value: item.stt, percent: item.stt / maxStt * 100 }))} locale={locale} />
    <UsageBars title="TTS usage · characters this period" values={providers.map((item) => ({ name: providerName(item.provider), value: item.tts, percent: item.tts / maxTts * 100 }))} locale={locale} />
  </section>;
}
...
function ActivityList({ title, page, total, pageSize, onPage, children }: { title: string; page: number; total: number; pageSize: number; onPage: (page: number) => void; children: React.ReactNode }) {
  ...
  return <section className="rounded-xl border border-indigo-950/10 bg-white p-5"><div className="flex items-center justify-between"><h2 className="text-lg font-bold text-[#1d1a5e]">{title}</h2><span className="text-xs font-bold text-slate-400">{page + 1} / {pages}</span></div><div className="mt-2 min-h-[240px]">{children}</div><div className="mt-3 flex justify-end gap-2"><button type="button" aria-label="Previous page" disabled={page === 0} onClick={() => onPage(page - 1)} className="h-9 w-9 rounded-lg border border-indigo-950/10 text-[#2a2586] disabled:opacity-30">←</button><button type="button" aria-label="Next page" disabled={page + 1 >= pages} onClick={() => onPage(page + 1)} className="h-9 w-9 rounded-lg border border-indigo-950/10 text-[#2a2586] disabled:opacity-30">→</button></div></section>;
}
