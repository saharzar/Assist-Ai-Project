...
export function BillScenarioShell({ currentStep, title, subtitle, compact = false, assistant, children }: {
  ...
  return (
    <section className={`catalogue-page min-h-screen px-5 sm:px-8 ${compact ? "py-3" : "py-8"}`}>
      <div className={`mx-auto grid w-full items-start gap-5 ${assistant ? "max-w-7xl xl:grid-cols-[minmax(0,1fr)_300px]" : "max-w-5xl"}`}>
      <div className="w-full overflow-hidden rounded-3xl border border-cyan-200/80 bg-white/95 shadow-[0_24px_60px_-34px_rgba(48,41,146,0.5)]">
